...
const Logup = () => {
   ...
   const signup = async (e) => {
      ...
      if (!e.target[0].value.replace(/ /g, "") || !e.target[1].value.replace(/ /g, "") || !e.target[2].value.replace(/ /g, "")) {
         return notification(setMsg, 'Tous les champs doivent être remplis.');
      };

      if (!e.target[2].value.replace(/ /g, "").match(valueOk)) {
   ...
   return (
      <main className={styles.login} >
         ...
         <section>
            ...
            <Link onClick={() => window.scrollTo(0, 0)} to={"/customer/login"} >Vous avez déjà un compte ?</Link>
