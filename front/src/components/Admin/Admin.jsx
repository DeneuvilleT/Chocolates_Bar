...
{/* Choice Cards */}
         ...
         <section>
            <FontAwesomeIcon icon={faCandyCane} title="Afficher les produits"
               onClick={() => {
                  setListProduct(true); dispatch(setAllProducts([]));
...
{/* List Customers */}
         ...
         <section>
            ...
            {listCustomer && <ul>{customers ? customers?.map(customer =>
               ...
               </li>) : <h4>Chargement des utilisateurs ...</h4>}
            </ul>}

{/* List Orders */}

            {listOrder && <ul> {orders ? orders?.map(order =>
               ...
               </li>) : <h4>Chargement des commmandes ...</h4>}
            </ul>}

{/* List Comments */}

            {listComment && <ul> {comments ? comments?.map((post) =>
               <li key={post.id}>
               ...
               </li>) : <h4>Chargement des commenatires ...</h4>}
            </ul>}

{/* List Products */}

            {listProduct && <>
               <button onClick={() => { !add ? setAdd(true) : setAdd(false); dispatch(setAllProducts([])) }}>Ajouter un produit</button>
               <input placeholder='recherche par nom ...' onChange={(e) => { search(e); setAll(false)}} type="text" />
               <button onClick={() => { recupDatas(getProducts, setAllProducts); setAll(true); setAdd(false) }}>Afficher tous les produits</button>


               <ul style={{ width: "90%", gap: '1vw' }}>
                  {(!all ? products : products.length) ? products?.map((product) =>
                     ...
                     </li>) : <h4>Chargement des produits ...</h4>}
               </ul>
