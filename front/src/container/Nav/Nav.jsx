...
const Nav = () => {
   ...
   return (
      <nav className={styles.nav}>
         <Link onClick={()=>window.scrollTo(0, 450)} style={{ borderBottom: `${location.pathname === '/' ? 'solid #b07f00 3px' : 'none'}`  }}
         ...
         <Link style={{ borderBottom: `${location.pathname === '/product/search' ? 'solid #b07f00 3px' : 'none'}` }}
            to={"/product/search"} onClick={() => { dispatch(saveDetail(null)); window.scrollTo(0, 230) }}>
            {screenSize.dynamicWidth > 650 ? 'Catalogue' : <FontAwesomeIcon icon={faCandyCane} />}
         </Link>

         <Link onClick={() => window.scrollTo(0, 200)} style={{ borderBottom: `${location.pathname === '/history' ? 'solid #b07f00 3px' : 'none'}` }}
            to={"/history"}>{screenSize.dynamicWidth > 650 ? 'Historique' : <FontAwesomeIcon icon={faBook} />}</Link>

         <Link onClick={() => window.scrollTo(0, 200)} style={{ borderBottom: `${location.pathname === '/cart' ? 'solid #b07f00 3px' : 'none'}` }}
